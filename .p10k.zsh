# Generated by Powerlevel10k configuration wizard on 2024-11-04 at 14:43 MST.
...
() {
  ...
  # Makes it so that the actual venv name is displayed if the name is venv, not the dir name
  # typeset -g POWERLEVEL9K_VIRTUALENV_GENERIC_NAMES=()
  typeset -g POWERLEVEL9K_VIRTUALENV_CONTENT_EXPANSION='$(
    if [[ -n "$VIRTUAL_ENV" ]]; then
        if [[ "$PWD" == "${VIRTUAL_ENV%/*}" ]]; then
            # In the virtual environment’s directory: Show the venv name and Python version
            echo "${VIRTUAL_ENV:t} $(python --version 2>&1 | cut -d " " -f 2)"
        else
            # Outside the virtual environment’s directory: Show the parent directory and venv name
            echo "${VIRTUAL_ENV:h:t} (${VIRTUAL_ENV:t})"
        fi
    fi
  )'
